...
type IdSpaceData = {
  ...
  updateSelectedLabel: (selectedNodeId: string) => void
  showDistanceColors: boolean
}

export function generateIdSpaceTree(layer: Konva.Layer ): IdSpaceData {
...
function updateTree(layer: Konva.Layer, data: IdSpaceData) {
  ...
  for (let i = 0; i < data.treeNodes.length; i++) {
    ...
    } else if (data.selectedNodeId === "" || !data.showDistanceColors) {
  ...
  if (data.updateSelectedLabel) {
    data.updateSelectedLabel(data.selectedNodeId)
  }
}
...
function IdSapceTree({ layer, updateSelectedLabel }: IProps) {
  ...
  useEffect(() => {
    console.log("once !!!!")
    const data = generateIdSpaceTree(layer.current)
    data.updateSelectedLabel = updateSelectedLabel
  }, [])
